...
ENGINEERING_GOAL_KICKOFF_TEMPLATE = (
    ...
    "During long work, keep a compact checkpoint with completed items, "
    "evidence gathered, open risks or approvals, and the next smallest "
    "safe action.\n\n"
    ...
    "specific evidence from files, commands, tests, PR state, runtime "
    "checks, or artifact paths."
)
...
ENGINEERING_GOAL_CONTINUATION_TEMPLATE = (
    ...
    "Completion audit before final answer:\n"
    "- map every explicit requirement to current evidence\n"
    "- treat missing, indirect, or stale evidence as incomplete\n"
    "- include changed files, commands, tests, PR state, runtime state, or artifact paths\n\n"
    "Blocked audit before stopping:\n"
...
JUDGE_SYSTEM_PROMPT = (
    ...
    "- The response includes concrete evidence that the goal was completed, "
    "such as changed files, command/test output, produced artifacts, PR state, "
    "or runtime checks, OR\n"
    "- The response clearly shows the final deliverable was produced with "
    "specific evidence.\n\n"
    "Do not accept generic phrases like 'done', 'all requirements met', "
    "'implemented successfully', or 'it works' unless the response also "
...
JUDGE_USER_PROMPT_TEMPLATE = (
    ...
    "Decision: Find concrete evidence in the agent's response that the "
    "goal is satisfied. Do not accept generic completion claims without "
    "specific evidence such as files changed, command output, test results, "
    "artifact paths, PR state, or runtime checks. If specific evidence is "
    "missing, the goal is NOT done — return CONTINUE.\n\n"

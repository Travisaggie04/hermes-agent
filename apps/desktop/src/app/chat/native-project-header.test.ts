...
describe('native project chat header', () => {
  ...
  it('lets the native chat header switch Jenny projects without opening Mission Control', () => {
    ...
    expect(source).toContain('queryFn: () => getMissionControlProjectSessions(NATIVE_PROJECT_SESSION_LIMIT)')
  ...
  it('shows project chat counts in the native header selector', () => {
    ...
    expect(source).toContain('nativeProjectChatModel')
    expect(source).toContain('const projectModel = useMemo')
    expect(source).toContain('const projects = projectModel.projects')
    expect(source).toContain("project.sessionCount > 0 ? `${project.sessionCount} ${project.sessionCount === 1 ? 'chat' : 'chats'}` : 'New draft'")
  ...
  it('lets Travis leave project mode from the native project picker', () => {
    ...
    expect(source).toContain('title="Other chats"')
    expect(source).toContain('onClearProject()')
    expect(source).not.toContain('<option value="">{loading ? \'Loading projects...\' : \'Other chats\'}</option>')
  })
  ...
  it('keeps the selected project visible when the project list is temporarily stale', () => {
    ...
    expect(source).toContain("const selectedLabel = selectedProjectTitle || (loading ? 'Loading projects...' : 'Other chats')")
    expect(source).toContain('<span className="min-w-0 truncate">{selectedLabel}</span>')
  })
  ...
  it('keeps the native project picker scoped to existing project records', () => {
    expect(source).toContain('nativeProjectChatModel')
    expect(source).toContain('nativeProjectChatModel(projectsQuery.data?.projects.map(item => item.record) ?? [], projectSessionsQuery.data?.groups ?? [])')
  })
  ...
  it('uses the blank native chat home as a project picker before a session exists', () => {
    ...
    expect(source).toContain('queryFn: () => getMissionControlProjectSessions(NATIVE_PROJECT_SESSION_LIMIT)')
    expect(source).toContain('const projectHomeModel = useMemo')
    expect(source).toContain('const projectHomeOptions = projectHomeModel.projects')
    ...
    expect(source).toContain('otherChatCount: projectHomeModel.otherChatCount')
    expect(source).toContain('otherChats: projectHomeModel.otherChats')
    expect(source).toContain('onResumeOtherSession: onResumeSession')
    expect(source).toContain('projectHomeSessionsQuery.refetch()')
    expect(source).toContain('onSelectProject: onStartProjectChat')
  })

  it('shows recent project sessions and unfiled Other chats inside the header switcher', () => {
    expect(source).toContain('otherChats={projectModel.otherChats}')
    expect(source).toContain('otherChatCount={projectModel.otherChatCount}')
    expect(source).toContain('onResumeOtherSession={onResumeSession}')
    expect(source).toContain('function ProjectSwitcherSessionRow')
    expect(source).toContain('onResumeProjectSession(session.id, project.id, project.name)')
    expect(source).toContain('onResumeOtherSession(session.id)')
  })
  ...
  it('lets the native chat header create a structured Jenny project brief', () => {
    ...
    expect(source).toContain('Name the work, give Jenny the goal, and keep advanced setup optional.')
    expect(source).toContain('createNativeProjectFromIntake(value,')
  ...
  it('uses the same clean project chat action for header, home, and newly created projects', () => {
    expect(source).toContain('onOpenProjectSession: (sessionId: string, projectId: string, projectName: string) => void')

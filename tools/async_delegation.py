...
def dispatch_async_delegation(
    *,
    ...
    interrupt_fn
        ...
        explicit cancel).
    max_async_children
        Concurrency cap. ``0`` disables dispatch. When at capacity the dispatch is REJECTED (the caller
    ...
    if max_async_children <= 0:
        return {
            "status": "rejected",
            "error": (
                "Async delegation is disabled. Set delegation.max_async_children "
                "to 1 or higher before dispatching background subagents."
            ),
        }
    record: Dict[str, Any] = {

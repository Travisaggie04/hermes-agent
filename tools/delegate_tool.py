...
_DEFAULT_MAX_ASYNC_CHILDREN = 0


def _get_max_async_children() -> int:
    """Read delegation.max_async_children from config (floor 0, no ceiling).

    ``0`` disables background delegation. ``1`` or greater caps how many
    background (``background=true``) subagents can run at once.
...
def delegate_task(
    ...
    async_max_children = _get_max_async_children() if background else None
    if background and (async_max_children is None or async_max_children <= 0):
        return tool_error(
            "Async delegation is disabled. Set delegation.max_async_children "
            "to 1 or higher in config.yaml before using background=true."
        )
    ...
    if n_tasks == 1:
        ...
        if background:
            ...
            dispatch = dispatch_async_delegation(
                ...
                max_async_children=async_max_children,
            )

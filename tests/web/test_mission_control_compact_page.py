...
def test_compact_route_has_project_rooms_and_record_draft_controls() -> None:
    ...
    for expected in [
        ...
        "Refresh replies",
        "flex h-[100dvh] min-h-0 w-full min-w-0 max-w-[100dvw] touch-pan-y flex-col overflow-hidden overscroll-x-none",
        "sr-only",
...
def test_compact_project_chat_wraps_long_mobile_text() -> None:
    ...
    for expected in [
        "overflow-x-hidden",
        "max-w-[100dvw]",
        "touch-pan-y flex-col overflow-hidden overscroll-x-none",
        ...
        "min-w-0 max-w-full",
        "w-fit max-w-[min(100%,calc(100dvw-1.5rem))]",
        "max-w-[min(100%,calc(100dvw-1.5rem))] rounded-lg border px-3 py-2 text-sm",

...
def test_compact_chat_bridge_controls_fail_closed_on_live_flags() -> None:
    ...
    for expected in [
        ...
        'compactExecutionLockReasons("hard_boundary_contract", hardBoundary)',
        ...
        "readOnly.eligible !== true || readOnly.preview_ready !== true",
        "scopedPr.eligible !== true || scopedPr.preview_ready !== true",
...
def test_compact_health_dashboard_fails_closed_on_execution_locks() -> None:
    ...
    for expected in [
        ...
        "function compactWorkspaceSafetyFlagReasons",
        ...
        'compactExecutionLockReasons("Orchestration run graph", runGraph)',
        'compactExecutionLockReasons("Child agent", childProjection)',
        'compactExecutionLockReasons("Child handoff", childInstruction)',
        'compactExecutionLockReasons("Report review queue", reportReviewQueue)',
        'compactExecutionLockReasons("Report contract", reportContract)',
        'compactExecutionLockReasons("Stop/cancel control", stopControl)',
        ...
        "workspaceSafetyFlagReasons.length === 0",
        ...
        "runGraphLockReasons.length",
        "childProjectionLockReasons.length",
        "childInstructionLockReasons.length",
        "reportReviewQueueLockReasons.length",
        "reportContractLockReasons.length",
        "stopControlLockReasons.length",
        "reportReviewQueueCount",
        "reportContractIncompleteCount",
        "stopCancelBlockedCount",
        "compactRecordText",
        ...
        "Orchestration run graph needs review",
        "Child-agent projection needs review",
        "Child handoff preview needs review",
        "Report review queue needs Jenny review",
        "Report contract needs review",
        "Stop/cancel control needs review",
        ...
        "Reports must include summary, result, risks or blockers, evidence, and tests",
        "Stopped or cancelled work stays manual-review-only",
        ...
        "${workspaceSafetyDetail}; active lanes=${activeLaneCount}.",
    ]:
        assert expected in src

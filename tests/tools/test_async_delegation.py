...
def test_dispatch_rejects_when_async_disabled():
    def runner():
        raise AssertionError("disabled async delegation must not run")

    res = ad.dispatch_async_delegation(
        goal="disabled", context=None, toolsets=None, role="leaf", model="m",
        session_key="", runner=runner, max_async_children=0,
    )
...
def test_delegate_task_background_routes_async_and_does_not_block(monkeypatch):
    ...
    with patch.object(dt, "_get_max_async_children", return_value=3), \
...
def test_delegate_task_background_disabled_rejects_before_child_build(monkeypatch):
    from unittest.mock import MagicMock, patch
    import tools.delegate_tool as dt
    ...
    with patch.object(dt, "_get_max_async_children", return_value=0), \
         patch.object(dt, "_build_child_agent",
                      side_effect=AssertionError("child should not be built")):
        out = dt.delegate_task(
            goal="should not launch", background=True, parent_agent=parent,
        )

    import json
    parsed = json.loads(out)
    assert "error" in parsed
    assert "Async delegation is disabled" in parsed["error"]

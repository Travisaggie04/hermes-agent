...
def _build_worker_node_run_record(payload: dict[str, Any]) -> WorkerNodeRunRecord:
    ...
    return WorkerNodeRunRecord(
        ...
        parent_run_id=parent_run_id,
        project_id=_workspace_text(payload.get("project_id"), max_chars=120),
        worker_id=_workspace_text(payload.get("worker_id"), max_chars=120),
        worker_type=_workspace_text(payload.get("worker_type"), max_chars=80) or "codex",
        display_name=_workspace_text(payload.get("display_name"), max_chars=160),
        ...
        capability_summary=_workspace_text(payload.get("capability_summary"), max_chars=MAX_WORKSPACE_TEXT_CHARS),
        capabilities_advertised=_workspace_list(payload.get("capabilities_advertised")),
        capabilities_allowed=_workspace_list(payload.get("capabilities_allowed")),
        capabilities_blocked=_workspace_list(payload.get("capabilities_blocked")),
        project_scope=_workspace_list(payload.get("project_scope")),
        lane_scope=_workspace_list(payload.get("lane_scope")),
        max_concurrent_read_only_lanes=_workspace_int(payload.get("max_concurrent_read_only_lanes")),
        max_concurrent_mutation_lanes=_workspace_int(payload.get("max_concurrent_mutation_lanes")),
        source_of_truth=_workspace_text(payload.get("source_of_truth"), max_chars=160) or "WorkerNodeRunRecord",
        safety_notes=_workspace_list(payload.get("safety_notes")),
        worker_dispatch_enabled=False,
